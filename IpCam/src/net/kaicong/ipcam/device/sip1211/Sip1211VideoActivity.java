package net.kaicong.ipcam.device.sip1211;
 ...
 */
public class Sip1211VideoActivity extends BaseSipDeviceActivity {
    ...
    public static final int PTZ_UP_DOWN = 18;
    public static final int PTZ_UP_DOWN_STOP = 20;
    public static final int PTZ_LEFT_RIGHT = 17;
    public static final int PTZ_LEFT_RIGHT_STOP = 19;
    public static final String KEY_BRIGHTNESS = "2";
    public static final String KEY_CONTRAST = "3";
    public static final String KEY_MODE = "9";
    public static final String KEY_FLIP = "8";
    public static final String KEY_MIRROR = "7";
    public static final String KEY_SATURATION = "5";
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        cgiControlParams.get1211ImageAttr(ip, port, account, password, new CgiControlParams.OnCgiTaskListener<CgiImageAttr>() {
        ...
        mVideoWidth = 640;
        mVideoHeight = 480;
        record = new Record(Sip1211VideoActivity.this, mVideoWidth, mVideoHeight, deviceId);
...
//                R.drawable.video_play_revert,
    ...
    public class DoRead extends AsyncTask<String, Void, MjpegInputStream> {
        ...
        protected void onPostExecute(MjpegInputStream result) {
            if (result == null) {
                Toast.makeText(Sip1211VideoActivity.this, getString(R.string.connstus_connection_failed), Toast.LENGTH_LONG).show();
                Sip1211VideoActivity.this.finish();
            } else {
//                if (record == null) {
//                    record = new Record(Sip1211VideoActivity.this, mVideoWidth, mVideoHeight, deviceId);
//                }
...
//                    break;
                    ...
                    if (isRecording) {
                        ...
                        if ((boolean) view.getTag() == true) {
                            ...
                            if (videoSettingPopWindow == null) {
                                videoSettingPopWindow = new VideoSettingPopWindow(Sip1211VideoActivity.this, cgiImageAttr, R.layout.pop_item_sip1211, this);
                            }
                            int[] xy = new int[2];
            ...
            switch (position) {
                case 0:
                    ...
                    Toast.makeText(Sip1211VideoActivity.this, getString(R.string.tips_snapshot_ok), Toast.LENGTH_SHORT).show();
                    break;
                case 1:
                    //录像
                    if ((boolean) view.getTag() == true) {
                ...
                case 2:
                    ...
                    if ((boolean) view.getTag() == true) {
                        //处于选中状态，点击后取消选中
                        videoBottomPopWindow.filterWhiteColor(position);
                        cgiControlParams.set1211PTZControl(ip, port, account, password, PTZ_UP_DOWN_STOP, false);
                    } else {
                        videoBottomPopWindow.filterRedColor(position);
                        cgiControlParams.set1211PTZControl(ip, port, account, password, PTZ_UP_DOWN, false);
                    }
                    break;
                case 3:
                    //左右巡航
                    if ((boolean) view.getTag() == true) {
                        //处于选中状态，点击后取消选中
                        videoBottomPopWindow.filterWhiteColor(position);
                        cgiControlParams.set1211PTZControl(ip, port, account, password, PTZ_LEFT_RIGHT_STOP, false);
                    } else {
                        videoBottomPopWindow.filterRedColor(position);
                        cgiControlParams.set1211PTZControl(ip, port, account, password, PTZ_LEFT_RIGHT, false);
                    }
                    break;
            }
        }
    ...
    public void onVideoModeSet(int position) {
        ...
        cgiControlParams.set1211ImageAttr(ip, port, account, password, KEY_MODE, String.valueOf(position + 1), null);
    }

    @Override
    public void onVideoBrightnessSet(int num) {
        super.onVideoBrightnessSet(num);
        cgiControlParams.set1211ImageAttr(ip, port, account, password, KEY_BRIGHTNESS, String.valueOf(num - 127), null);
    }

    @Override
    public void onVideoSaturationSet(int num) {
        super.onVideoSaturationSet(num);
        cgiControlParams.set1211ImageAttr(ip, port, account, password, KEY_SATURATION, String.valueOf(num), null);
    }

    @Override
    public void onVideoContrastSet(int num) {
        super.onVideoContrastSet(num);
        cgiControlParams.set1211ImageAttr(ip, port, account, password, KEY_CONTRAST, String.valueOf(num), null);
    }
    ...
    public void onVideoFlipSet(boolean on) {
        ...
        String value = on ? "1" : "0";
        cgiControlParams.set1211ImageAttr(ip, port, account, password, KEY_FLIP, value, null);
    }
    ...
    public void onVideoMirrorSet(boolean on) {
        ...
        String value = on ? "1" : "0";
        cgiControlParams.set1211ImageAttr(ip, port, account, password, KEY_MIRROR, value, null);
    }
    ...
    protected void onPtzLeft() {
        super.onPtzLeft();
        cgiControlParams.set1211PTZControl(ip, port, account, password, PTZ_LEFT, true);
    }

    @Override
    protected void onPtzRight() {
        super.onPtzRight();
        cgiControlParams.set1211PTZControl(ip, port, account, password, PTZ_RIGHT, true);
    }

    @Override
    protected void onPtzUp() {
        super.onPtzUp();
        cgiControlParams.set1211PTZControl(ip, port, account, password, PTZ_UP, true);
    }

    @Override
    protected void onPtzDown() {
        super.onPtzDown();
        cgiControlParams.set1211PTZControl(ip, port, account, password, PTZ_DOWN, true);
    }

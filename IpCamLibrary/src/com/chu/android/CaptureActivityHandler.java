...
public final class CaptureActivityHandler extends Handler {
    ...
    private State state;
    private final com.chu.android.camera.CameraManager cameraManager;
    ...
    CaptureActivityHandler(CaptureActivity activity,
                           ...
                           String characterSet,
                           com.chu.android.camera.CameraManager cameraManager) {
        this.activity = activity;
        decodeThread = new DecodeThread(activity, decodeFormats, characterSet,
                new com.chu.android.ViewfinderResultPointCallback(activity.getViewfinderView()));
        decodeThread.start();
        state = State.SUCCESS;
    ...
    public void handleMessage(Message message) {
        int msgInt = message.what;
        if (msgInt == R.id.restart_preview) {
            LogUtil.d(TAG, "Got restart preview message");
            restartPreviewAndDecode();
        }
        if (msgInt == R.id.decode_succeeded) {
        ...
        if (msgInt == R.id.decode_failed) {
            // We're decoding as fast as possible, so when one decode fails, start another.
            state = State.PREVIEW;
            cameraManager.requestPreviewFrame(decodeThread.getHandler(), R.id.decode);
        }
        if (msgInt == R.id.return_scan_result) {
            LogUtil.d(TAG, "Got return scan result message");
            activity.setResult(Activity.RESULT_OK, (Intent) message.obj);
            activity.finish();
        }
        if (msgInt == R.id.launch_product_query) {

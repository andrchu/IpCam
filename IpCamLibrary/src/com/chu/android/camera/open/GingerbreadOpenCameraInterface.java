...
public final class GingerbreadOpenCameraInterface implements OpenCameraInterface {
    ...
    public Camera open() {
        ...
        if (numCameras == 0) {
            com.chu.android.LogUtil.w(TAG, "No cameras!");
            return null;
        }
        ...
        Camera camera;
        if (index < numCameras) {
            com.chu.android.LogUtil.i(TAG, "Opening camera #" + index);
            camera = Camera.open(index);
        } else {
            com.chu.android.LogUtil.i(TAG, "No camera facing back; returning camera #0");

...
//                }
    ...
    void setScale(float scale) {
        int disX = (int) (this.getWidth() * Math.abs(1 - scale)) / 3;// 获取缩放水平距离
        int disY = (int) (this.getHeight() * Math.abs(1 - scale)) / 3;// 获取缩放垂直距离
        ...
        // 缩小
        else if (scale < 1 && this.getWidth() > screen_W) {

            //缩放边界判断
            //←
            if (this.getLeft() < 0) {
                if (this.getRight() > start_Right) {
                    current_Left = this.getLeft() + disX;
                    current_Right = this.getRight() - disX;
                } else {
            ...
            //↑
            if (this.getTop() < start_Top) {
                current_Top = this.getTop() + disY;
            } else {
                current_Top = start_Top;
            }
            //↓
            if (this.getBottom() > start_Bottom) {
                current_Bottom = this.getBottom() - disY;
            } else {
            ...
            //设置显示位置
            if ((current_Left > start_Left && (current_Top >= start_Top || current_Bottom <= start_Bottom)) ||
                    (current_Right < start_Right && (current_Top >= start_Top || current_Bottom <= start_Bottom))) {
                this.setFrame(start_Left, start_Top, start_Right,
                        start_Bottom);
            } else {
                this.setFrame(current_Left, current_Top, current_Right,
                        current_Bottom);
